...
<form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <div>
        ...
        <textarea name="short_desc" id="short_desc" rows="3" class="w-full border-gray-300 rounded" required>
            {{ old('short_desc', $news->short_desc) }}
        </textarea>
    </div>

    <div>
        <label for="content" class="block font-medium">Isi Berita</label>
        <textarea name="content" id="content" rows="5" class="w-full border-gray-300 rounded" required>
    ...
    <div>
        ...
        @if ($news->image)
            <img src="{{ asset('images/'.$news->image) }}" alt="Gambar Berita" style="max-width: 200px; margin-bottom: 10px;">
        @else
